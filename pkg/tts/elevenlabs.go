...
func (e *ElevenLabs) Synthesize(ctx context.Context, text string) (*AudioResult, error) {
	...
	// Include output_format query parameter for PCM
	outputFormat := e.encodingToAPIFormat()
	url := fmt.Sprintf("%s/text-to-speech/%s?output_format=%s", e.baseURL, e.config.VoiceID, outputFormat)
...
func (e *ElevenLabs) encodingToAPIFormat() string {
	switch e.config.OutputFormat {
	case EncodingPCM16:
		return "pcm_16000"
	case EncodingPCM22:
