...
// SetPose implements batched pose control (Issue #135)
func (m *mockRobot) SetPose(head *Offset, antennas *[2]float64, bodyYaw *float64) error {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.poseCalls = append(m.poseCalls, struct {
		head     *Offset
		antennas *[2]float64
		bodyYaw  *float64
	}{head, antennas, bodyYaw})
	return nil
}
...
func (m *mockRobot) lastHead() (roll, pitch, yaw float64) {
	m.mu.Lock()
	defer m.mu.Unlock()
	// Check poseCalls first (new batched API)
	if len(m.poseCalls) > 0 {
		last := m.poseCalls[len(m.poseCalls)-1]
		if last.head != nil {
			return last.head.Roll, last.head.Pitch, last.head.Yaw
		}
	}
...
func (m *mockRobot) poseCallCount() int {
	m.mu.Lock()
	defer m.mu.Unlock()
	return len(m.poseCalls)
}
...
func TestController_RunStop(t *testing.T) {
	...
	// Should have made some calls (now uses batched SetPose)
	if mock.poseCallCount() < 5 {
		t.Errorf("Expected at least 5 pose calls, got %d", mock.poseCallCount())
	}
}
...
func TestController_Rate(t *testing.T) {
	...
	// Now uses batched SetPose instead of individual calls
	count := mock.poseCallCount()
	if count < 8 || count > 15 {
		t.Errorf("Expected ~10 pose calls at 100Hz over 100ms, got %d", count)
	}
}
